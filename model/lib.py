...
class Order_Head(nn.Module):
    def __init__(self, n_channel, n_frame, n_joint, n_person, h_channel=256, **kwargs):
        ...
        self.order_U = nn.Sequential(nn.Conv2d(self.n_frame * (h_channel // n_frame) // 2, 256, kernel_size=1),
                                      nn.ReLU(True),
                                      nn.Conv2d(256, 128, kernel_size=1))
        self.order_V = nn.Sequential(nn.Conv2d(self.n_frame * (h_channel // n_frame) // 2, 256, kernel_size=1),
                                      nn.ReLU(True),
                                      nn.Conv2d(256, 128, kernel_size=1))
